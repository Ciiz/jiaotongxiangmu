...
<div style="background:#ffffff;height:600px;overflow-y:scroll">
  <div>
    <Collapse class="evaluate-Collapse" v-model="value1">
      <Panel :name="index+1" v-for="(item,index) in student_exam.object" v-if="item.length!==0" :key="index">
        <span>
        ...
        <p slot="content">
          <Row class="evaluate-Collapse-Row" v-for="(item2,index2) in item" :key="index2">
            ...
            <Col :span="6" class="evaluate-select-anwser">
              ...
              <div>
                <span>TA的答案</span>
                <span :class="item2.user_answer===item2.answer?'evaluate-select-anwser-rightBoreder':'evaluate-select-anwser-errorBoreder'">{{item2.user_answer===''?'未作答':item2.user_answer}}</span>
              </div>
            </Col>
          </Row>
        </p>
          ...
          <Row v-for="(item2,index2) in item" :key="index2" style="padding:16px 0">
            <p class="evaluate-main-question">{{index2+1}}.{{item2.content}}（{{item2.score}}分）</p>
             <Row type="flex" justify="space-between" class="evaluate-main-evaluate">
              ...
              <Col>
                <span>本题得分</span>
                <Input type="text" v-model="score_option_list[item2.id][item2.score_option_list_id]" style="width:106px" class="evaluate-main-score-input"/>
              </Col>
            </Row>
