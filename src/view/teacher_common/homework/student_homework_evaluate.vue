<template>
  <div class="modal-content" style="color:#666666">
    ...
    <div class="homework-evaluate-anwser" v-if="student_homework.submit_status"
      v-html="student_homework.student_homework_content"></div>
    <div v-if="student_homework.file_url" style="font-size:14px;margin-bottom:10px">
      文件:<a :href="student_homework.file_url" target="blank">{{student_homework.file_name || '————附件————'}}</a>
    </div>
    <div v-for="score_item in student_homework.score_option" :key="score_item.index">
      ...
      <Input type="text" :label="score_item.option_name" v-model="score_item.score" style="width:106px"
        class="evaluate-main-score-input" />
    </div>
    <Input type="textarea" class="homework-evaluate-textarea" :rows="4" v-model="student_homework.evaluation"
