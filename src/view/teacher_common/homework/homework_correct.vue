<template>
  <div>
    ...
    <Modal v-model="modal" :title="title" :width="modal_width" footer-hide>
      <student-exam-list :targetwork_id="target_id" :target_type="'student_homework_list'" v-if="target === 'student_homework_list'" @on-refresh-parent-list="getData()"></student-exam-list>
    </Modal>
  </div>
</template>
<script>
import { get_homework_release_list } from '@/api/data'
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            return (
              <div>
                <Tag color="greed" v-show={row.correct_status}>已批改</Tag>
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            return (
              <div>
                <Button size="small" type="primary" onClick={() => { this.open('student_homework_list', row.homework_release_id, `${row.class_name}-作业: ${row.homework_name}`) }}>批改</Button>
              </div>
            )
          }
        }
