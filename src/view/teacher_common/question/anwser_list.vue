  ...
  <div class="answer_list">
     <Tabs v-model="t" @on-click='changeType'>
      <TabPane label="提问内容" name="name2">
        ...
        <div style="margin-top:16px;font-size:14px;color:#26282F;float:right">提问时间：{{getDate(mainQuestion.created_at)}}</div>
      </TabPane>
        ...
        <div class="answer_ul" style="margin-bottom:20px">
          <Row class="answer_l" v-for="(item) in answerList" :key="item.id">
            ...
            <Col :span="3">
              ...
              <Button type="primary" size="small" style="float:right;transform:translateY(-3px)" @click="change(item.id,item.score)">修改</Button>
            </Col>
          </Row>
        </div>
      </TabPane>
    </Tabs>
    <Modal v-model="isshowscore" :width="300" title="请输入评分" footer-hide>
...
<style>
  ...
  .answer_list .ivu-tabs-tabpane::-webkit-scrollbar { width: 0 !important }
