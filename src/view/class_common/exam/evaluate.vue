<template>
  <Row class="modal-content">
    ...
    <Row>
      <h3>
        <Divider orientation="left"><span style="font-weight:bold">主观题：</span><span style="font-size: 12px;">得分: <Tag
              color="green">{{student_exam.subject_score}}</Tag> 分</span></Divider>
      </h3>
      <div v-for="subject in student_exam.subject" :key="subject.id" class=" margin-b-15 row-item">
        ...
        <a :href="subject.student_answer.video_url" target="blank"
          v-show="subject.student_answer.video_url">{{subject.student_answer.video_url | dealFileUrl}}</a>
        <h3>评分：</h3>
        ...
        <Row>
          ...
          <Score :label="option.option_name" :disabled="true" v-model="option.score">
          </Score>
          </Col>
        </Row>
    ...
    <Row>
      <h3>
        <Divider orientation="left"><span style="font-weight:bold">客观题：</span><span style="font-size: 12px;">得分: <Tag
              color="green">{{student_exam.object_score}}</Tag> 分</span></Divider>
      </h3>
      <table class="my-table">
        ...
        <tbody>
          <tr v-for="object in student_exam.object" :key="object.id" class="item-row row-item">
            ...
            <td width="20%">
              <span :class="{ 'wrong': !object.is_right, 'right': object.is_right}"
                class="answer-item">{{object.student_answer.content.join(',')}}</span>
            </td>
            <td width="20%">
