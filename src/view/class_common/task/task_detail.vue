      ...
      <Tabs size="small" name="task_detail" v-model="tab">
        <TabPane label="课中任务" v-if="student_task" tab="task_detail" name="task">
          ...
          <Row>
            ...
            <Button style="float:right;margin-top:15px;" size="small" type="primary" @click="submitTask" :disabled="student_task.score_status === 1">{{submitTxt}}</Button>
          </Row>
          </div>
        ...
        <TabPane label="其他小组任务完成情况" tab="task_detail" v-if="show_team_list" name="other_team">
          <TeamList :student_task_id="student_task_id" :self_submit_status="student_task.submit_status" v-if="show_team_list"></TeamList>
        </TabPane>
      </Tabs>
  </div>
</template>
