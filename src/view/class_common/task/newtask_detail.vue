<template>
  <div class="newtask_detail">
    ...
    <div style="color:#888888;word-break:break-all" v-html="student_task.task.content"></div>
    <Divider orientation="left" v-if="student_task.task_release.release_type===1"><span class="black-b">小组成员</span></Divider>
    <ul class="newtask_detail-ul" v-if="student_task.task_release.release_type===1">
      <li v-for="(item,index) in student_task.team_members_info" :key="index">
        <img :src="item.icon" style="width:30px;height:30px;border-radius:50%;vertical-align:middle;margin-right:10px"/>
        <span>
          <span>{{item.name}}</span>
