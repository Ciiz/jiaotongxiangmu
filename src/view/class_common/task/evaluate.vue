  ...
  <Row>
    <Row type="flex" justify="space-between" class="evaluate-top">
      ...
      <Col>
        ...
        <span style="color:#222222">个人成绩：<span style="color:#EE3333">{{student_task.score_info.user_score===null?'未评分':student_task.score_info.user_score}}</span></span>
      </Col>
    </Row>
    ...
    <div style="margin-bottom:30px;margin-top:20px" v-if="student_task.file_url!==''">
      ...
      <a :href="student_task.file_url" class="new-file" target="blank">{{student_task.file_name?student_task.file_name:'附件'}}</a>
    </div>
    ...
    <Col span="24">
        ...
        <div v-if="student_task.task_release.release_type === 1">
          ...
          <Row v-for="member in members" :key="member.id">
            ...
            <h4>内容</h4>
            <div class="evaluate-content" v-html="member.submit_content" v-if="member.submit_type === 0"></div><!--在线编辑的-->
            <div class="evaluate-content" v-else><!--文件上传的-->
          ...
          <div  v-if="student_task.is_team_leader"><!-- 组长（显示组员对组长的评价） -->
           <Divider/>
