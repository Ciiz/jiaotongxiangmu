...
<div class="student-note">
  <notePreview :id="this.id" :courseware_id="this.courseware_id" :currentIdIndex="''" @unshowPreviewNote="unshowPreviewNote" v-show="previewNote"></notePreview>
    ...
    <div class="ab-scroll">
      <ul>
        <li v-for="(item,index) in list" :key="index" class="note-li">
          ...
          <div>
            <span style="color:#BBBBBB;font-size:14px">{{moment(item.created_at * 1000).format('YYYY-MM-DD HH:mm')}}</span>
            <Icon type="md-close" color="#FF3333" size="24" style="margin-left:20px;cursor:pointer" @click="showDeleteNote(item)"/>
          </div>
        </li>
      </ul>
    </div>
