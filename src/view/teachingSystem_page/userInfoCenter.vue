  ...
  <div class="userInfoCenter">
    <div style="position:relative;width:100%;height:100%">
      <Form ref="formValidate" :model="infolist" :label-width="80" style="padding: 18px 18px 10px 18px">
        <FormItem label="头像:">
          <ImageCropUpload :imgsrc="infolist.icon" @success="(url)=>{this.infolist.icon = url}"
            :params="{type: 'icon',token:$store.state.user.token}" :size_editable="false" :imgWidth='300'
            :imgHeight='300'>
          </ImageCropUpload>
        </FormItem>
        <Row>
        ...
        <FormItem label="推流地址" style="margin-bottom:0">
          ...
          <Button size="small"
            style="margin-left:10px;background:#0BA97E;outline:none;color:#ffffff;border-radius: 15px"
        ...
        <FormItem label="摄像头地址">
          ...
          <Button size="small"
            style="margin-left:10px;background:#0BA97E;outline:none;color:#ffffff;border-radius: 15px"
            @click="copyText2()">复制</Button>
        </FormItem>
        <Button @click="submit()"
          style="width:82px;float:right;background:#3B9BFF;outline:none;color:#ffffff;border-radius: 15px;">修改</Button>
      </Form>
    </div>
  </div>
</template>
