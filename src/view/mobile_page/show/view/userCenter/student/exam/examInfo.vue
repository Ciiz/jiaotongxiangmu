<template>
  <div class="m-examInfo flex-contain">
    <mt-header :title="student_exam.exam.course_name+' '+(student_exam.exam.type===1?'课前':(student_exam.exam.type===2?'课中':'课后'))+'测试'" style="border-bottom:1px solid #CCCCCC">
      <router-link to="/homeworkExamIndex" slot="left">
        <mt-button icon="back"></mt-button>
      </router-link>
      <img slot="right" src="@/assets/images/mobile_student/setting.png" style="width:0.33rem"  @click="popupVisible=true">
    </mt-header>
    <mt-popup
      class="m-popup"
      ...
      <div class="m-popup-contain">
        <div><span>{{student_exam.exam.course_name}} {{student_exam.exam.type===1?'课前':(student_exam.exam.type===2?'课中':'课后')}}测试</span></div>
        ...
        <div><span>接收时间：</span>{{moment(student_exam.exam_release.released_at * 1000).format('YYYY-MM-DD HH:mm:ss')}}</div>
    ...
    <div class="m-examInfo-contain hideScroll flex-contain">
      <div class="m-examInfo-obj">
        ...
        <div class="m-examInfo-obj-list">
          ...
          <div v-for="(item,index) in objectList" :key="index" class="m-examInfo-obj-item" :class="item.student_answer.content!==''?'m-examInfo-obj-item-active':'m-examInfo-obj-item-normal'" @click="showObject(index)">
            {{index+1}}
          </div>
        </div>
      </div>
      <div class="m-examInfo-subj">
        <div v-for="(item,index) in subjectList" :key="index" class="m-examInfo-subj-item">
          <div style="margin-bottom:0.28rem">
            ...
            <img src="@/assets/images/mobile_student/writeExam.png" style="width:0.4rem;float:right;margin:0.1rem 0.2rem 0 0" @click="showSubject(index,item)">
          </div>
          <div style="position:relative">
            <div class="content-shadow"></div>
            <div class="m-examInfo-subj-item-answer" v-html="item.student_answer.content===''?'未作答':item.student_answer.content"></div>
          </div>
        </div>
      </div>
    </div>
